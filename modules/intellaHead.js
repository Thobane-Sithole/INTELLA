...
class TalkingHead {
  ...
  */
  constructor(node, opt = null) {
    ...
    this.streamAutoLipsyncOffset = 0;
    this.streamAutoLipsyncSampleRate = 24000;
  }
  ...
  */
  ...
  async streamStart(opt = {}, onAudioStart = null, onAudioEnd = null, onSubtitles = null, onMetrics = null) {
    ...
    this.streamAutoLipsyncOffset = 0;
    this.streamAutoLipsyncSampleRate = opt.sampleRate || 24000;
  ...
  */
  ...
  streamInterrupt() {
    ...
    // Instantly silence any already-buffered audio via gain ramp (like reference impl)
    if (this.audioStreamGainNode && this.audioCtx) {
      try {
        this.audioStreamGainNode.gain.linearRampToValueAtTime(0, this.audioCtx.currentTime + 0.05);
        // Restore gain after buffered audio is gone
        setTimeout(() => {
          try { this.audioStreamGainNode.gain.setValueAtTime(1, this.audioCtx.currentTime); } catch (e) { }
        }, 200);
      } catch (e) { }
    }
  ...
  */
  streamAudio(r) {
    ...
    } else if (r.audio !== undefined) {
      // Auto-generate lip sync from PCM amplitude when no viseme/word data provided
      this._autoLipsyncFromPCM(r.audio);
    }
  }


  /**
  * Auto-generate lip sync visemes from PCM audio amplitude.
  * Used when streaming raw audio without pre-computed viseme data.
  * Analyzes amplitude in small segments and maps to mouth visemes.
  * @param {ArrayBuffer|Int16Array|Uint8Array|Float32Array} audio Raw PCM audio data
  * @private
  */
  _autoLipsyncFromPCM(audio) {
    // Convert to Int16Array for amplitude analysis
    let int16;
    if (audio instanceof Int16Array) {
      ...
      int16 = new Int16Array(audio.length);
      for (let i = 0; i < audio.length; i++) {
        int16[i] = Math.max(-1, Math.min(1, audio[i])) * 0x7fff;
      }
    } else {
      return;
    }
    ...
    const sr = this.streamAutoLipsyncSampleRate;
    const segmentMs = 25; // Analyze in 25ms segments for smooth lip movement
    const segmentSamples = Math.floor(sr * segmentMs / 1000);

    const visemes = [];
    const vtimes = [];
    const vdurations = [];

    for (let offset = 0; offset < int16.length; offset += segmentSamples) {
      const end = Math.min(offset + segmentSamples, int16.length);
      let sumSquares = 0;
      ...
      // Voice activity: generate viseme when sound detected
      if (rms > 0.015) {
        const level = Math.min(1.0, rms * 6);
        // Map amplitude to different mouth shapes for natural look
        let viseme;
        if (level > 0.6) viseme = 'aa';       // Wide open mouth
        else if (level > 0.35) viseme = 'O';   // Medium rounded open
        else if (level > 0.15) viseme = 'E';   // Medium open
        else viseme = 'I';                       // Slightly open

        const segTime = this.streamAutoLipsyncOffset + (offset / sr * 1000);
        const segDur = ((end - offset) / sr * 1000);

        visemes.push(viseme);
        vtimes.push(segTime);
        vdurations.push(segDur);
      }
    }

    // Update time offset for next chunk
    this.streamAutoLipsyncOffset += (int16.length / sr * 1000);

    if (visemes.length > 0) {
      const lipsyncData = { visemes, vtimes, vdurations };

      if (this.streamWaitForAudioChunks && !this.streamAudioStartTime) {
        // Queue until playback starts
        if (this.streamLipsyncQueue.length >= 200) {
          this.streamLipsyncQueue.shift();
        }
        this.streamLipsyncQueue.push(lipsyncData);
      } else {
        if (!this.streamAudioStartTime) {
          this.streamAudioStartTime = this.animClock;
        }
        this._processLipsyncData(lipsyncData, this.streamAudioStartTime);
      }
    }
  }
